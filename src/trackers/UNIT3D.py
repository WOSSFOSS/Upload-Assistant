...
class UNIT3D:
    ...
    async def search_existing(self, meta: dict[str, Any], _: Any) -> list[dict[str, Any]]:
        ...
        def append_result(each: dict[str, Any], check_pending: bool, other_upload: bool = False) -> None:
            if check_pending:
            ...
            result_key = (str(torrent_id or ""), str(link or ""), str(name or ""))
            ...
            async with httpx.AsyncClient(timeout=10.0, follow_redirects=True) as client:
                for url in urls_to_check:
                    ...
                    if "api/torrents/pending" in url:
                        check_pending = True
                    searches: list[tuple[ParamsList, bool]] = [(request_params, False)]
                    if tuple(request_params) != tuple(other_request_params):
                        searches.append((other_request_params, True))

                    for search_params, other_upload in searches:
                        response = await client.get(url=url, headers=headers, params=search_params)
                        response.raise_for_status()

                        if response.status_code == 200:
                            data = response.json()
                            for each in data.get("data", []):
                                append_result(each, check_pending, other_upload=other_upload)
                        else:
